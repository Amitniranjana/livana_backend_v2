...
use crate::models::chat::{ChatUser, ChatChannel};
...
impl ChatService {
    ...
    pub async fn create_app_instance_user(&self, app_instance_arn: &str, user_id: &str, name: &str) -> Result<ChatUser> {
        let resp = self.identity_client
    ...
    pub async fn create_channel(&self, app_instance_arn: &str, name: &str, mode: &str, privacy: &str, cleaner_arn: &str) -> Result<ChatChannel> {
        ...
        let resp = self.messaging_client
            .create_channel()
            .app_instance_arn(app_instance_arn)
            .name(name)
            .mode(mode_enum)
    ...
    pub async fn add_channel_flow(&self, channel_arn: &str, member_arn: &str, chimer_bearer: &str) -> Result<()> {
    ...
    pub async fn send_message(&self, channel_arn: &str, content: &str, sender_arn: &str) -> Result<String> {
        let resp = self.messaging_client
            .send_channel_message()
            .channel_arn(channel_arn)
            .content(content)
            .r#type(aws_sdk_chimesdkmessaging::types::ChannelMessageType::Standard)
            .persistence(aws_sdk_chimesdkmessaging::types::ChannelMessagePersistenceType::Persistent)
