...
use crate::dtos::kyc::{
    KycResponse, KycRow, KycStatusResponse, KycSubmitRequest, KycUpdateRequest, UploadResponse,
};
...
pub async fn upload_profile(
    ...
    while let Ok(Some(field)) = multipart.next_field().await {
        if field.name().unwrap_or("") == "file" {
            ...
            if let Err(e) = app_state
                .kyc_service
                .storage()
                .upload_file(&key, data.to_vec(), &content_type)
                .await
            {
                return (
            ...
            let bucket = std::env::var("KYC_BUCKET_NAME")
                .unwrap_or_else(|_| "livana-kyc-documents".to_string());
    ...
    (
        StatusCode::BAD_REQUEST,
        Json(json!({"success":false,"message":"No file field found in request"})),
    )
}
...
pub async fn upload_document(
    ...
    (
        StatusCode::BAD_REQUEST,
        Json(json!({"success":false,"message":"No file field found"})),
    )
}
...
pub async fn delete_upload(
    ...
    match result {
        Ok(r) if r.rows_affected() > 0 => (
            StatusCode::OK,
            Json(
                json!({"success":true,"message":"File deleted successfully","data":{"file_id":file_id}}),
            ),
        ),
        Ok(_) => (
...
pub async fn submit_kyc(
    ...
    let now = Utc::now();
    let services_json =
        serde_json::to_value(payload.services.unwrap_or_default()).unwrap_or(json!([]));
...
pub async fn get_kyc(
    ...
    (
        StatusCode::OK,
        Json(json!({"success":true,"message":"KYC details fetched","data":{"kyc":kyc}})),
    )
}
...
pub async fn get_kyc_status(
    ...
    (
        StatusCode::OK,
        Json(json!({"success":true,"message":"KYC status fetched","data":resp})),
    )
}
...
pub async fn update_kyc(
    ...
    let owner: Option<Uuid> =
        match sqlx::query_scalar("SELECT user_id FROM kyc_submissions WHERE id = $1")
            .bind(kyc_id)
            .fetch_optional(&app_state.db)
            .await
        {
    ...
    if let Some(v) = &payload.mobile_number {
        qb.push(", mobile_number = ");
        qb.push_bind(v);
    }
    if let Some(v) = &payload.email_id {
        qb.push(", email_id = ");
        qb.push_bind(v);
    }
    ...
    if let Some(v) = &payload.date_of_birth {
        qb.push(", date_of_birth = ");
        qb.push_bind(v);
    }
    if let Some(v) = &payload.profile_picture_url {
        qb.push(", profile_picture_url = ");
        qb.push_bind(v);
    }
    if let Some(v) = &payload.apartment_name {
        qb.push(", apartment_name = ");
        qb.push_bind(v);
    }
    ...
    if let Some(v) = &payload.landmark {
        qb.push(", landmark = ");
        qb.push_bind(v);
    }
    ...
    if let Some(v) = &payload.permanent_address {
        qb.push(", permanent_address = ");
        qb.push_bind(v);
    }
    if let Some(v) = payload.is_permanent_same_as_present {
        qb.push(", is_permanent_same_as_present = ");
        qb.push_bind(v);
    }
    ...
    if let Some(v) = &payload.govt_id_type {
        qb.push(", govt_id_type = ");
        qb.push_bind(v);
    }
    if let Some(v) = &payload.govt_id_number {
        qb.push(", govt_id_number = ");
        qb.push_bind(v);
    }
    if let Some(v) = &payload.govt_id_document_url {
        qb.push(", govt_id_document_url = ");
        qb.push_bind(v);
    }
    ...
    if let Some(v) = &payload.experience_document_url {
        qb.push(", experience_document_url = ");
        qb.push_bind(v);
    }
    ...
    if let (Some(doc_url), Some(full_name)) = (&payload.govt_id_document_url, &payload.full_name) {
        ...
        qb.push(", extracted_name = ");
        qb.push_bind(extracted_name);
        qb.push(", name_match_score = ");
        qb.push_bind(score);
        qb.push(", verification_status = ");
        qb.push_bind(v_status);
    }
    ...
    (
        StatusCode::OK,
        Json(
            json!({"success":true,"message":"KYC updated successfully","data":{"kyc_id":kyc_id.to_string()}}),
        ),
    )
}
...
async fn run_ocr_on_document_url(
    ...
    let score: f64 = if norm_extracted.contains(&norm_input) || norm_input == norm_extracted {
        ...
        let extracted_words: std::collections::HashSet<&str> =
            norm_extracted.split_whitespace().collect();
        let common = input_words.intersection(&extracted_words).count();
        if input_words.is_empty() {
            0.0
        } else {
            common as f64 / input_words.len() as f64
        }
    };
